...
def do_chunk ( chunk )
  ...
  chunk.each do |m|
    m.chomp
    thrs[i] = Thread.new {
      cmd = %( ssh #{m} "  cd /export/content/;  ls -1 glu/apps/*/i0[0-9][0-9]/logs/gc.log; ls -1 */i0[0-9][0-9]/logs/gc.log " 2>/dev/null )
      res = `#{cmd}`  
      puts; puts m + "============ \n" 
      machine = m
      res.split.each { |ret|
...
# TODO:
#        machine = m
        app, instance = nil, nil

        if ret =~ /glu\/apps\/(.*?)\/(.*?)\/.*/ or  ret =~ /(.*?)\/(.*?)\/.*/ 
          app = $1
          instance = $2
        end
		...
		app != 'app' and
		not app =~ /\.bak/  and
		not app =~ /DECOMMISSION/ and 
		not app =~ /OBSELETE/  and
		not File.exists?( drop_dir )
...
def main_all
  ...
  # all hosts
  all_hosts = MYCM.get_hosts_of('ech3', 'array')

  all_hosts.each_slice($parallel) do |chunk|
