...
def getoptions():
  ...
  parser.add_option(    "-e", "--env", dest="env",
                        help="[required] Environment name, currently supported envs: stg, beta, ech3, ei1, ei3, ela4")
  ...
  if options.env == None:
    print "ERROR: I need env name, e.g., stg, beta, ech3, ei1, ei3, ela4"
    parser.print_help()
    sys.exit(1)
...
def main():
  ...
  if env == 'stg' or env == 'beta':
    ...
    print "ERROR: env %s not allowed. Supported env: stg, beta, ech3, ei1, ei3, ela4" % env
    sys.exit(1)
