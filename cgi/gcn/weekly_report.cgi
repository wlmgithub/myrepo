...
sub display_customized_query {
  ...
  print "<fieldset>";
  print "<legend> Weekly  Report  Query </legend>";
...
<select name="number_of_weeks" id="number_of_weeks">
  <option selected='selected' value='2'> Last 2 weeks </option>
...
###### gchart data part
  ...
  for  (0 .. $#dates_from) {
    my $date_from = $dates_from[$_];
    my $date_to = $dates_to[$_];
    my $show_date = get_show_date($date_to);
    $gchart .= qq(
...
sub get_dates_ref_of_last_x_weeks {
  
  my $x = shift;
  my @dates;
  
  foreach (1..$x) {
    push @dates, strftime("%Y-%m-%d",localtime(time-86400*7*$_));
  }
...
if ( param("submit") and param("submit") eq "Submit" ) {
  ...
  my $number_of_weeks = param("number_of_weeks");

#  print "number_of_weeks: $number_of_weeks<p>";

  my $ref = get_dates_ref_of_last_x_weeks($number_of_weeks);
